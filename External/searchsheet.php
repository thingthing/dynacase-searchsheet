...
function getdetailledsearch($dbaccess, $name)
{ //:SSH_IDSEARCH,SSH_SEARCH,SSH_IDFAMILY,SSH_FAMILY
...
function getSearchAttribute($dbaccess, $famid, $name)
{ //SSH_IDACOL,SSH_ACOL,SSH_LCOL
    ...
    while (list($k, $v) = each($tinter)) {
        if (($name == "") || (preg_match("/$pattern_name/i", $v->labelText, $reg))) $tr[] = array(
            $v->labelText,
            $v->id,
            $v->labelText
        );
