...
*/
Class _SEARCHSHEET extends Doc
{
    /*
     * @end-method-ignore
    */
    ...
    function getHTMLReport($filters = "", $sort = "", $limit = "", $page = "", $type = "html", $tids = false)
    {
        ...
        foreach ($taids as $k => $v) {
            $cols[] = array(
                "color" => ($tbgcolor[$k] == "") ? "inherit" : $tbgcolor[$k],
                "attribute" => $v,
        ...
        while ($v = $s->nextDoc()) {
            ...
            foreach ($cols as $kc => $vc) {
                if ($vc["function"]) {
                    ...
                    if (substr($ft, 0, 2) == '::') {
                        $cells[$kc] = array(
                            "content" => $v->applyMethod($ft, "", -1, array(
                                $v->getRValue($vc["attribute"])
                            ))
                        );
                    } else {
                        if (function_exists($ft)) {
                            ...
                            $cells[$kc] = array(
                                "content" => sprintf(_("method or function %s not exists") , $vc["function"])
                            );
                        }
                    }
                } else {
                    if ($vc["attribute"]) {
                        if (strstr($vc["attribute"], ":")) {
                            $cells[$kc] = array(
                                "content" => $v->getRValue($vc["attribute"], "", true, ($type == 'html'))
                            );
                        } else {
                            if ($type == 'html') $cells[$kc] = array(
                                "content" => $v->getHtmlAttrValue($vc["attribute"], '_blank')
                            );
                            else {
                                $oa = $v->getAttribute($vc["attribute"]);
                                $cells[$kc] = array(
                                    "content" => $v->getValue($vc["attribute"])
                                );
                ...
                if ($vc["dyncolor"]) {
                    $ft = $vc["dyncolor"];
                    if ($ft) {
                        $cells[$kc]["color"] = $v->applyMethod($ft, "", -1, array(
                            $cells[$kc]["content"]
                        ));
                    }
